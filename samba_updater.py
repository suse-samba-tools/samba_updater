...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run against obs samba package to update to latest version. This will branch the target package into a new project under your home project, then check it out on the local machine. It also downloads the latest package version from samba.org, then clones a copy of samba (unless an existing clone is specified with --samba) and generates a changelog entry. Finally, an updated package will be checked into your new subproject in obs.', epilog='Example:\n\n./samba_updater.py 4.11 network:samba:STABLE\n', formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument('-A', '--apiurl', help='osc URL/alias', action='store', default='https://api.opensuse.org')
    parser.add_argument('--skip-test', action='store_true', default=False, help='Use to disable build testing of new sources (this will submit to the build service without testing!)')
    ...
    parser.add_argument('SAMBA_VERSION', help='The relative samba version (ex. 4.11)')
    parser.add_argument('SOURCEPROJECT', help='The source obs project to branch from')
    parser.add_argument('SOURCEPACKAGE', help='The source obs package[s] to update (default=[talloc, tdb, tevent, ldb])', nargs='*', default=['talloc', 'tdb', 'tevent', 'ldb'])
